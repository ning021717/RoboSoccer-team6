...
class ForwardRight(SoccerRobot):
    def run(self):
        ...
        while self.robot.step(TIME_STEP) != -1:
            ...
            # A vision miss is a normal sensor outcome, not an exception.
            # Rotate to reacquire; do not substitute Supervisor ball truth.
            if ball is None:
    ...
    def _am_i_the_chaser(self, ball, me):
        ...
        other = robots.get("BLUE_FW_L", None)
        if other is None:
            return True  # 如果没有左前锋机器人，允许右前锋追球
    ...
    def _avoid_teammates(self, me, min_d=0.40):
        robots = self.world.get("robots", {})
        for mate in ("BLUE_FW_L", "BLUE_DEF"):
