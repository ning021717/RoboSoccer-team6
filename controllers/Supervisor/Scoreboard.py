...
class Scoreboard:
  ...
  def _record_event(self, supervisor, kind, ball):
    self.events.append({
      "time_s": round(supervisor.getTime(), 4),
      "event": kind,
      "ball_gt": [round(value, 5) for value in ball],
      "score": {"red": self.redTeamScore, "blue": self.blueTeamScore},
    })
  ...
  def checkBall(self, supervisor):
    ...
    if abs(ballCoordinate[0]) > 4.5:
      if abs(ballCoordinate[1]) < 1.35:
        if supervisor.ballPriority == "N":
          if 4.5 < ballCoordinate[0]:
            ...
            self._record_event(supervisor, "red_goal", ballCoordinate)
            ...
            self._record_event(supervisor, "blue_goal", ballCoordinate)
        ...
        if 4.5 < ballCoordinate[0]:
          ...
          self._record_event(supervisor, "ball_out_blue_end", ballCoordinate)
        else:
          supervisor.setBallPriority("R")
          supervisor.setBallPosition(BALL_POSITIONS["OUT_R"])
          self._record_event(supervisor, "ball_out_red_end", ballCoordinate)
      ...
      self._record_event(supervisor, "ball_out_touchline", ballCoordinate)
